...
export const ComparisonDataPoint: React.FC<{
  ...
  function tryFormatting(originalValue, formatString, defaultValue) {
    try {
      return SSF.format(formatString, originalValue);
    } catch (err) {
      return defaultValue;
    }
  }
    ...
    <ComparisonDataPointGroup>
      ...
      {config[`comparison_style_${compDataPoint.name}`] !== 'value_change' ? null : (
        <ComparisonPercentageChange data-value={valueChange} onClick={() => { handleClick(compDataPoint, event) }}>
          ...
          {tryFormatting(valueChange, config[`comp_value_format_${compDataPoint.name}`], valueChange.toLocaleString('en-US'))}
        </ComparisonPercentageChange>
      )}
      {config[`comparison_style_${compDataPoint.name}`] !== 'value' ? null : 
      <ComparisonSimpleValue onClick={() => { handleClick(compDataPoint, event) }}>
        {config[`comp_value_format_${compDataPoint.name}`] === "" ? compDataPoint.formattedValue : tryFormatting(compDataPoint.value, config[`comp_value_format_${compDataPoint.name}`], compDataPoint.formattedValue)}
